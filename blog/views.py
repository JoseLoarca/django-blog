...
def starting_page(request):
    all_posts = sorted(posts_dict, key=get_date)
...
def posts(request):
    return render(request, 'blog/all-posts.html', {'posts': posts_dict})


def post_details(request, slug):
    post = next(post for post in posts_dict if post['slug'] == slug)
